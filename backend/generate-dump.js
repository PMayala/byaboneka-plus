...
const IGNORE_FILES = new Set([
  ...
  ".DS_Store", "Thumbs.db", OUTPUT_FILE, "generate-dump.js"
]);

const ALLOWED_EXTENSIONS = new Set([
  ".ts", ".js", ".tsx", ".jsx", ".json", ".md",
  ".yml", ".yaml", ".sql", ".graphql", ".gql",
  ".sh", ".css", ".html", ".ejs", ".hbs", ".env.example"
]);

const SPECIAL_FILES = new Set([
  "Dockerfile", "Procfile", "Makefile", ".gitignore",
...
function getLanguage(filePath) {
  ...
  const map = {
    ...
    ".sql": "SQL", ".css": "CSS", ".html": "HTML", ".sh": "Shell"
  };
  return map[ext] || "Text";
}
...
function buildTree(dir, prefix = "", rootDir = dir, depth = 0, maxDepth = 4) {
...
function main() {
  ...
  out.push(`  Languages:       ${Object.entries(langCounts).map(([k, v]) => `${k} (${v})`).join(", ")}`);
  out.push("");
  ...
  console.log(`  🔒 Secrets excluded (.env, .env.production, .env.backup)`);
  console.log(`  🗑️  Junk excluded (node_modules, coverage, uploads, etc.)`);
  console.log("");
}
