...
const HomePage: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Count actual returned items instead of approximating
      const returnedCount = (foundRes.data.data || []).filter(
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20 lg:py-28 relative">
          <div className="text-center max-w-3xl mx-auto">
            <Badge variant="verified" size="lg" className="mb-4 sm:mb-6">
              <Star className="w-4 h-4 mr-1" /> {t('home.badge')}
            </Badge>
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4 sm:mb-6 leading-tight">
              {t('home.heroTitle')}{' '}
              <span className="text-accent-400">{t('home.heroHighlight')}</span>
            </h1>
          ...
          <div className="mt-12 sm:mt-16 grid grid-cols-3 gap-4 sm:gap-8 max-w-2xl mx-auto">
            {[
              ...
              { value: stats.returned, label: t('home.itemsReturned') },
              { value: '146+', label: t('home.partnerCoops') },
            ].map((stat, i) => (
      ...
      <section className="bg-white border-b sticky top-16 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <form onSubmit={handleSearch} className="flex gap-2 sm:gap-4">
            ...
            <Button type="submit" size="md" className="hidden sm:flex">{t('common.search')}</Button>
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">{t('home.whyChoose')}</h2>
            <p className="text-gray-600 max-w-2xl mx-auto">{t('home.whyChooseSubtitle')}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              { icon: <Search className="w-7 h-7" />, title: t('home.smartMatching'), desc: t('home.smartMatchingDesc'), color: 'bg-blue-500' },
              { icon: <Shield className="w-7 h-7" />, title: t('home.secureVerification'), desc: t('home.secureVerificationDesc'), color: 'bg-green-500' },
              { icon: <Building2 className="w-7 h-7" />, title: t('home.cooperativeNetwork'), desc: t('home.cooperativeNetworkDesc'), color: 'bg-purple-500' },
              { icon: <TrendingUp className="w-7 h-7" />, title: t('home.trustSystem'), desc: t('home.trustSystemDesc'), color: 'bg-orange-500' },
            ].map((feature, i) => (
      ...
      <section className="py-12 sm:py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">{t('home.howItWorks')}</h2>
            <p className="text-gray-600">{t('home.howItWorksSubtitle')}</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {[
              { step: '01', title: t('home.stepReport'), desc: t('home.stepReportDesc'), icon: FileText },
              { step: '02', title: t('home.stepMatch'), desc: t('home.stepMatchDesc'), icon: Search },
              { step: '03', title: t('home.stepVerify'), desc: t('home.stepVerifyDesc'), icon: Shield },
              { step: '04', title: t('home.stepReturn'), desc: t('home.stepReturnDesc'), icon: CheckCircle },
            ].map((item, i) => (
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">{t('home.recentlyReported')}</h2>
              <p className="text-gray-600">{t('home.recentlyReportedSubtitle')}</p>
            </div>
            <Link to="/search">
              <Button variant="outline" rightIcon={<ArrowRight className="w-4 h-4" />}>
                {t('home.viewAllItems')}
                  ...
                  <Card hover className="overflow-hidden h-full">
                    <div className="aspect-video bg-gray-100 flex items-center justify-center relative">
                      ...
                      <Badge variant={isFound ? 'verified' : 'pending'} className="absolute top-2 left-2">
                        {isFound ? t('home.found') : t('home.lost')}
                      </Badge>
                    </div>
                    <div className="p-4">
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gradient-to-r from-primary-600 to-primary-700">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-4">{t('home.readyToStart')}</h2>
          <p className="text-primary-100 mb-8 text-base sm:text-lg">{t('home.readyToStartSubtitle')}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {isAuthenticated ? (
              <>
                <Link to="/dashboard"><Button size="lg" variant="accent">{t('home.goToDashboard')}</Button></Link>
                <Link to="/report-lost"><Button size="lg" variant="secondary" className="bg-white/10 border-white/30 text-white hover:bg-white/20">{t('home.reportLostItem')}</Button></Link>
              </>
            ) : (
              <>
                <Link to="/register"><Button size="lg" variant="accent">{t('home.createFreeAccount')}</Button></Link>
                <Link to="/login"><Button size="lg" variant="secondary" className="bg-white/10 border-white/30 text-white hover:bg-white/20">{t('home.signIn')}</Button></Link>
              </>
            )}
          </div>
        </div>
