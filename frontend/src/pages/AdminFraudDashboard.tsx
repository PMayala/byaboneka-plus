...
const AdminFraudDashboard: React.FC = () => {
  ...
  const loadFlaggedUsers = async () => {
    try {
      ...
      setError(t('admin.loadFraudError'));
    } finally {
      setLoading(false);
    }
  };
  const getRiskBadge = (events: number) => {
    if (events >= 5) return { label: t('admin.critical'), bg: '#FEE2E2', color: '#991B1B' };
    if (events >= 3) return { label: t('admin.high'), bg: '#FFE4E6', color: '#9F1239' };
    return { label: t('admin.medium'), bg: '#FEF3C7', color: '#92400E' };
  };
  ...
  if (loading) {
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        <p style={{ color: '#6B7280' }}>{t('admin.loadingFraud')}</p>
      </div>
    );
  }
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 16 }}>
      ...
      {users.length === 0 ? (
        <div style={{
          ...
          <p style={{ color: '#065F46', fontWeight: 500 }}>{t('admin.noFlaggedUsers')}</p>
          <p style={{ color: '#6B7280', fontSize: 13 }}>{t('admin.noFlaggedUsersDesc')}</p>
        </div>
      ) : (
        <div style={{
          ...
          <div style={{
            ...
            <span>{t('admin.trustScoreLabel')}</span>
